...
describe("the canonical fixtures carry no system results", () => {
  ...
  it("fails closed for non-official, ineligible or unverified cells", () => {
    ...
    const baseCell = {
      ...
      totalEligibleCostUsd: 100,
      attemptArtifacts: Array.from({ length: 20 }, (_, index) => ({
        attemptId: `test-attempt-${index + 1}`,
        taskId: "test-task",
        taskVersion: "v1",
        redactedTraceId: `trace-test-attempt-${index + 1}`,
        finalStateEvidenceId: `final-test-attempt-${index + 1}`,
        invocationRecordIds: [`invocation-test-attempt-${index + 1}`],
