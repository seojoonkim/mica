...
function cell(overrides: Partial<RunCell> & Pick<RunCell, "system" | "country" | "family">): RunCell {
  ...
  return runCellSchema.parse({
    ...
    attemptArtifacts: Array.from({ length: eligibleRuns }, (_, index) => ({
      attemptId: `synthetic-attempt-${index + 1}`,
      taskId: "synthetic-task",
      taskVersion: "v1",
      redactedTraceId: `synthetic-trace-${index + 1}`,
      finalStateEvidenceId: `synthetic-final-state-${index + 1}`,
      invocationRecordIds: [`synthetic-invocation-${index + 1}`],
