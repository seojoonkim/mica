...
describe("run cell cross-field invariants", () => {
  ...
  function coherentCell(overrides: Record<string, unknown> = {}) {
    return {
      ...
      totalEligibleCostUsd: 1000,
      attemptArtifacts: Array.from({ length: 4 }, (_, index) => ({
        attemptId: `coherent-attempt-${index + 1}`,
        taskId: "coherent-task",
        taskVersion: "v1",
        redactedTraceId: `coherent-trace-${index + 1}`,
        finalStateEvidenceId: `coherent-final-state-${index + 1}`,
        invocationRecordIds: [`coherent-invocation-${index + 1}`],
