...
1. 이 디렉터리의 `READY.json`, `INPUT-MANIFEST.json`, `need-observations.staging.jsonl`, `observation-reviews.staging.jsonl`, 이 계약, `CLOSURE.template.json`만 읽는다.
2. observation과 review가 4행씩 1:1로 대응하고 같은 순서를 유지하는지 확인한다.
3. review의 다섯 checks가 모두 `pass`이고 verdict가 `accept`인 observation만 선택한다.
4. 선택된 원본 observation의 12개 필드와 값을 원래 순서대로 복사하고, 아래 세 필드만 뒤에 붙인다.
5. 결과는 `frozen-observations.staging.jsonl` 한 파일에 UTF-8 JSONL로 작성한다.
6. 작성 뒤 `CLOSURE.json`을 작성하고 정지한다.
