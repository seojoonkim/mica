...
| jobType | 저작자가 만드는 것 |
|---|---|
| `catalog-annotation` | 동결 후보에 카테고리·슬롯 상태·종료 유형·복잡도·목표 접점 부착 |
...
- 카테고리는 요청의 소재나 근거 수집처가 아니라 **주 성공 경로의 완료 조건과 권위 있는 최종 상태가 성립하는 생활 영역**을 우선한다. 두 영역이 실제로 겹쳐 근거만으로 가를 수 없으면 hold로 남긴다.
- 슬롯이 없다는 이유로 다른 카테고리에 밀어 넣지 않고, 슬롯이 많이 남았다는 이유로 그쪽으로 끌어당기지 않는다. **어느 카테고리가 비어 있든 판정 근거가 아니다.**
- 판정한 카테고리에 빈 슬롯이 있으면 `slotDisposition: assigned`와 가용 `proposedSlotId`를 쓴다. 그 카테고리가 가득 찼으면 `slotDisposition: category-overflow`, `proposedSlotId: null`로 보존한다. overflow 후보는 검토가 끝나도 100칸을 점유하지 않으며, 이후 교체·확장 판단의 재고로 남는다.
