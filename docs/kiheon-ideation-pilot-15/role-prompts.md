...
> fixture, reset, eligibility, oracle, frozen candidate를 전수 대조한다. 저작자가 `scripts/mica-measurement-asset.py validate`를 통과시킨 v3 자산이면 수치 불일치(worstPathProof↔simultaneityGate)·판정 클래스 이중 선언(sinks)·근거 없는 금지 상태 결속·terminal 도달 가능성 대리 검사 4개는 기계가 이미 확인했다는 뜻이다. 그래도 이 4개를 신뢰하지 말고 값 자체의 타당성(수치가 실제로 맞는지, `justifiedBy` 참조가 실제로 그 상황을 정당화하는지)은 직접 재확인한다 — 기계 검사는 필드가 서로 일치하는지만 보고 그 값이 옳은지는 보지 않는다. 기대 판정표의 모든 행이 같은 trace에서 동시에 성립하는지, gate와 locked path가 1:1인지, 실패 상태가 재검사에서도 유지되는지 확인한다. EXP literal label 전부가 registry에 있는지, 누락 성분 formula가 전 variant에서 같은지, 엄격한 사건 순서가 사건 부재를 true로 처리하지 않는지 확인한다. 판정은 위에서 아래로 첫 일치 규칙을 적용하고 규정 threshold와 사용자 값의 차이만으로 source conflict라 하지 않는다. oracle이 사용자 결과·권위 있는 readback·승인·금지·안전 인계를 판정하는 최소 조건인지 확인하고 특정 표현·클릭 순서·내부 레코드를 부당하게 강제하면 reject한다. 모든 파일 참조와 분기가 실제로 결속되고 reset이 결정적이며 자격 판정이 fail-closed일 때도 blind-agent rehearsal 전에는 `designable-pending-exposure`로만 판정한다. live 실행, 시장 성립, 공개 적격은 판정하지 않는다.
