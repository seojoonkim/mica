...
권장 사양은 최상위 추론 모델의 높은 추론 설정이며, Codex에서는 GPT-5.6 Sol `xhigh` 이상을 기준으로 한다. 최소 사양은 최신 상위권 도구 사용 모델의 `high`다. 모든 배치는 모델명, 모델 버전 또는 alias, 추론 설정, 실행 시각, 역할별 context ID를 기록한다. 모델이 달라도 계약과 산출물 형식은 같아야 하며, 낮은 사양 결과는 권장 사양 reviewer가 재검토한다.
