...
현재 `measurable-candidate`는 15건이다. 목표 100건까지 남은 85건도 같은 관문으로 단계적으로 제작한다. 후속 실행은 기존 표준 5건 방식과 별도 Lean 3건 방식 중 하나를 시작 전에 선택한다. Lean에서 결함이나 판정 충돌이 발견되면 표준으로 복귀하고, 100건에 도달한 뒤에는 전수 중복·coverage·측정·시장·실행 적합성을 다시 분석한다.
...
2. 팀의 목적과 위험에 따라 표준 5건 또는 Lean 3건을 선택해 근거 조사부터 측정 판정까지 전체 관문을 반복한다.
3. Lean에서 새 공정 결함이 없고 이전 결함의 회귀가 통과하면 다음 Lean 또는 표준 5건을 검토한다.
4. Lean에서 결함이 재발하거나 판정이 충돌하면 표준 방식으로 돌아가 계약과 검증을 보완한다.
