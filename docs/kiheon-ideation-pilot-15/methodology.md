...
4. 웹 진행판·팀 커뮤니케이션: 설계 수와 실행 수를 분리 표기하고 `0회 실행`을 성공률·실패율로
   읽히게 하지 않는다.
5. 자연어 도구 입력: 실행 도구는 고지나 최종 보고를 특정 정답 문장과 문자 단위로 비교하지 않는다.
   도구는 선행 상태·승인·제출 결속만 fail-closed로 강제하고, 요구 사실의 포함 여부는 숨겨진
   evaluator가 실행 뒤 판정한다. 특정 문구를 맞혀야 다음 단계가 열리면 하네스 결함으로 기록한다.
