...
export const ko = {
  ...
  home: {
    ...
    positionIntro:
      "WebArena, WebVoyager, WebShop, AndroidWorld, AppWorld, τ-bench는 이미 과제가 끝났는지를 결과로 재고 있으며, MICA는 그 성과를 부정하지 않고 그 위에 섭니다. 다만 조립된 소비자 에이전트, 일상 소비 볼일, 그리고 현지 채널과 본인확인, 결제 수단을 갖춘 살아 있는 시장이라는 좁은 조합을 함께 다루는 벤치마크는 아직 없습니다. MICA는 그 자리를 재려고 만드는 중이며, 아직 아무것도 재지 않았습니다.",
    positionItems: [
      {
        term: "빠져 있던 단위",
        detail:
    ...
    axesIntro:
      "기준을 통과한 과제는 점수 하나를 받습니다. 100 × 정확도 × 속도 × 비용이고, 각 인자는 초나 달러가 아니라 0과 1 사이로 맞춘 값입니다. 공개 비용은 현지 구매 금액이 아니라 모든 시장에서 USD로 통일한 평가 실행비입니다. 세 축은 원래 단위 그대로 따로 싣습니다. 점수는 기록을 줄여 보여 줄 뿐 기록을 대신하지 않습니다. 아직 점수를 받은 과제는 없습니다.",
