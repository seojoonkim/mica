...
export const ko = {
  ...
  site: {
    ...
    definition:
      "MICA는 버전을 고정한 소비자 에이전트 시스템을 아시아와 중동 6개 시장의 일상 용무를 기준으로 평가합니다. 실행은 격리된 모의 환경이 아니라 현지 채널과 본인확인, 동의 절차, 결제 수단, 앱 전용 흐름을 지나갑니다. 기준을 통과한 과제는 정규화 정확도·정규화 속도·정규화 비용을 곱해 점수 하나를 받고, 세 축의 원값은 그 옆에 나란히 공개합니다.",
    edition: "프리뷰 0.1",
    demoNotice:
  ...
  home: {
    ...
    frameworkFieldDetail:
      "100개 표준 과제가 6개 현지 시장에서 MICA의 현재 관찰 환경을 이룹니다. 10개 과제 영역은 예약·주문·변경·행정 등 여러 서비스를 거치는 실제 용무를 정의하고, 각 과제는 선언된 완료 조건과 승인 경계를 가집니다. 에이전트 시스템은 입력 해석부터 계획 수립, 모델·도구 선택, 상태 유지, 현지화, 안전한 인계와 복구까지 한 번의 종단 간 실행으로 평가됩니다. 실행 기록에서 확인된 최종 상태로 정확도를, 성공 실행의 경과 시간으로 속도를, 성공당 평가 비용으로 비용을 산출하고, 호출 계보와 상태 전이를 7개 진단 축에 연결합니다. 따라서 이는 서로 분리된 10개 API 테스트 묶음이 아니라 전체 에이전트 시스템을 관찰하는 시험장입니다.",
    ...
    positionIntro:
      "WebArena, WebVoyager, WebShop, AndroidWorld, AppWorld, τ-bench는 과제 완료를 결과로 측정합니다. MICA는 이를 확장해 조립된 소비자 에이전트가 현지 채널, 본인확인, 결제 수단을 갖춘 시장에서 일상 용무를 완료하는 능력을 평가합니다.",
    positionItems: [
      {
        term: "빠져 있던 단위",
        detail:
          "MICA의 측정 단위는 용무가 거쳐 간 서비스를 모두 아우르는 종단 간 결과입니다. 모델의 답변도, 도구가 반환한 응답도, 단일 화면의 성공도 아닙니다. 예약이나 주문, 취소, 변경이 실제로 이루어져야 결과로 인정합니다. 같은 용무가 로그인 단계에서 중단되었다면 부분 성공이 아니라 실패입니다.",
      },
      {
        term: "시장이 곧 과제",
        detail:
          "현지 채널, 본인확인과 휴대폰 인증, 결제 수단, 말투, 무엇을 완료로 보느냐는 번역 문제가 아니라 과제의 조건입니다. 같은 용무도 시장이 바뀌면 다른 문제가 됩니다. 따라서 하나의 벤치마크를 여섯 언어로 번역하지 않고 여섯 개의 시장 에디션을 별도로 운영합니다.",
      },
      {
    ...
    readinessItems: [
      ...
      },
      {
        term: "근거 구조",
        detail:
          "근거의 단위는 실행 묶음입니다. 묶음마다 고유 주소가 있고 내용을 다 밝히며, 무엇을 보여 줄 수 없는지도 적습니다. 구조는 구축되었지만 내용은 비어 있습니다. 기록된 묶음이 없습니다.",
      },
      {
        term: "공개 요건",
        detail:
  ...
  countries: {
    ...
    standfirst:
      "국가 에디션은 하나의 벤치마크를 다른 언어로 옮긴 것이 아닙니다. 각 에디션은 그 시장에서 실행을 실제로 좌우하는 채널, 결제 인프라, 신원 확인, 주소 형식, 서비스 관행에 맞춰 별도로 작성합니다. 따라서 같은 용무도 여섯 시장에서 서로 다른 조건을 지니게 됩니다.",
  ...
  tasks: {
    ...
    crossingTitle: "하나의 용무가 여러 서비스를 거칩니다",
    crossingIntro:
    ...
    scoringAggregationDetail:
      "적격 실행에서 파생한 과제 점수의 산술평균입니다. 사전 등록한 표준 과제 집합이 완결되지 않으면 점수를 비공표합니다. 제외 항목은 분모에서 조용히 사라지지 않고 사유와 함께 남깁니다.",
  ...
  methodology: {
    ...
    scoringAggregationDetail:
      "해당 계열 또는 시장의 적격 실행에서 파생한 과제 점수의 산술평균입니다. 사전 등록한 표준 과제 집합이 완결되지 않으면 비공표합니다. 제외 항목은 분모에서 조용히 사라지지 않고 사유와 함께 남깁니다.",
