...
export const ko = {
  ...
  site: {
    ...
    tagline:
      "결과가 검증되는 소비자 에이전트의 엔드투엔드 실행을 6개 국가별 현지 실생활 시장과 여러 일상 소비자 영역에서 평가하도록 설계된 세계 최초의 공개 벤치마크 이니셔티브.",
    secondary:
      "지금까지의 벤치마크는 지식 노동, 소프트웨어 개발, 벤치마크가 통제하는 웹 환경, 그리고 앱 하나로 격리된 시뮬레이션에서 모델과 에이전트를 측정해 왔습니다. MICA는 조립된 시스템이 소비자가 실제로 살아가는 시장에서 일상적인 볼일을 끝까지 처리할 수 있는지를 측정합니다.",
    definition:
      "MICA는 버전을 고정한 완성 상태의 소비자 에이전트 시스템을 아시아와 중동 6개 시장의 일상 볼일로 평가합니다. 실행은 통제된 샌드박스가 아니라 현지 채널, 본인확인과 동의 절차, 결제 수단, 앱 전용 흐름을 거칩니다. 검증된 과제는 정규화된 정확도·속도·비용 성분을 곱해 하나의 최종 점수를 받으며, 세 축의 원값은 그 옆에 따로 공개됩니다.",
    edition: "프리뷰 에디션 0.1",
    demoNotice:
  ...
  home: {
    ...
    noResultsDetail:
      "MICA는 어떤 시스템도, 어떤 점수도, 어떤 순위도 공개한 적이 없습니다. 지금 갖춰진 것은 측정의 틀뿐입니다. 아직 잠정 후보 상태인 10개 계열의 과제 분류 체계, 공개된 측정 방법, 기준값을 아직 정하지 않은 공개 요건, 그리고 앞으로의 모든 수치를 이름 붙은 집계 실행 셀까지 되짚게 하는 증거 모델입니다. 제출된 스냅샷이 독립적으로 재실행되고 이 요건을 통과한 뒤에야 결과가 이곳에 나타납니다.",
    positionEyebrow: "MICA가 존재하는 이유",
    positionTitle: "소비자의 볼일이 실제로 끝났는지를 재는 벤치마크는 아직 없습니다",
    positionIntro:
      "그동안 벤치마크의 관심은 기업의 지식 노동, 소프트웨어 개발, 벤치마크가 통제하는 웹 환경, 그리고 모바일이나 거래를 격리해 놓은 시뮬레이션에 몰려 있었습니다. 이 선행 연구들은 분명한 성과이며 MICA도 그 위에 서 있습니다. 다만 조립된 소비자 에이전트 시스템이 살아 있는 현지 시장에서 일상적인 볼일을 끝까지 마칠 수 있는지는 그 어느 쪽도 답하지 않습니다. MICA는 바로 그 빈자리를 측정하기 위해 만들어지고 있으며, 아직 아무것도 측정하지 않았습니다.",
    positionItems: [
      {
        term: "빠져 있던 평가 단위",
        detail:
          "평가 단위는 볼일이 거쳐 가는 모든 서비스를 아우른 소비자 결과 전체입니다. 모델의 답변도, 도구 호출의 응답도, 인터페이스 하나 안에서의 성공도 아닙니다. 예약이나 주문, 취소, 변경이 실제로 이루어져야 하나의 결과이며, 같은 볼일이 로그인 장벽에서 멈췄다면 그것은 부분 성공이 아닙니다.",
      },
      {
        term: "시장은 과제의 일부",
        detail:
          "현지 채널, 본인확인과 휴대폰 인증, 결제 수단, 언어와 말투, 인계와 완료를 규정하는 현지의 의미 체계는 번역 상의 세부가 아니라 과제의 조건 그 자체입니다. 같은 볼일도 시장마다 다른 문제가 됩니다. MICA가 하나의 벤치마크를 여섯 언어로 옮기는 대신 여섯 개의 에디션을 두는 이유입니다.",
      },
      {
        term: "앱 중심 아시아가 중요한 이유",
        detail:
          "앱 중심 시장에서는 탐색과 조회는 공개되어 있어도 상태를 바꾸는 층은 그렇지 않은 경우가 많습니다. 주문, 메시지, 결제, 계정 변경은 플랫폼 등록, 제휴 승인, 동의 범위, 미니앱 런타임, 메시지나 결제 인계를 거칩니다. 조회에서 멈추는 벤치마크는 정작 어려운 지점에 닿지 못합니다.",
      },
    ],
    positionLink: "MICA가 존재하는 이유 읽기 →",
    axesEyebrow: "과제마다 점수 하나, 세 축은 그대로 공개",
    axesTitle: "MICA는 정확도·속도·비용을 곱해 과제 점수를 냅니다",
    axesIntro:
      "검증된 과제는 하나의 최종 점수를 받습니다. 100 × 정확도 × 속도 × 비용이며, 각 인자는 원래의 초나 달러가 아니라 0과 1 사이로 정규화된 성분입니다. 아래 세 축은 고유 단위 그대로 공개됩니다. 점수는 기록을 요약할 뿐 기록을 대신하지 않기 때문입니다. 아직 점수가 매겨진 과제는 없습니다.",
    ...
    readinessItems: [
      {
        term: "과제 분류 체계",
        detail:
          "일상 과제 10개 계열이며, 표준 과제마다 선언된 최종 상태와 확인 경계가 있습니다. 공개 과제 세트의 잠정 후보로 공개했습니다. 초안이며, 아직 검증되지 않았고 시장별로 구분되지도 않았습니다.",
      },
      {
        term: "측정 방법",
        detail:
          "정확도·속도·비용을 각각 따로 공개하는 세 결과로 정의하고, 정규화한 성분을 곱해 검증된 과제마다 하나의 최종 점수를 만듭니다. 계열과 국가 수치는 유효한 과제 점수들의 산술평균입니다. 초안으로 공개되어 있으며 반론을 받습니다.",
      },
      {
        term: "증거 인프라",
        detail:
  ...
  tasks: {
    ...
    eyebrow: "과제 정의",
    title: "과제는 대화 기록이 아니라 소비자의 세계에서 일어난 변화입니다",
    standfirst:
      "모든 MICA 과제는 그 실행이 인정되기 위해 세계가 어떤 상태여야 하는지, 그리고 시스템이 어느 선을 넘기 전에 멈추고 물어야 하는지를 선언합니다. 측정하는 것은 올바른 중단과 인계까지 포함한 엔드투엔드 변화입니다. 그럴듯한 문장도, 성공한 도구 호출도, 인터페이스 하나 안에서의 초록 불도 결과가 아닙니다.",
    crossingEyebrow: "하나의 과제가 넘나드는 것",
    crossingTitle: "하나의 볼일, 여러 개의 실행 표면",
    crossingIntro:
      "소비자의 볼일 하나는 좀처럼 한자리에 머물지 않습니다. 공개 검색에서 시작해 앱 전용 흐름으로 이어지고, 현지 신원 확인이나 동의 절차에서 멈춰 섰다가, 결제나 메시지 인계를 지나, 그 상태를 보관하는 서비스에서 결과를 되읽을 수 있을 때에야 끝납니다. 이 경계 하나하나가 로그에서는 성공처럼 보이면서 실제로는 실패할 수 있는 지점입니다. 완료를 시스템이 마지막으로 받은 응답이 아니라 선언된 최종 상태로 정의하는 이유입니다. 아래 정의는 잠정 후보이며, 검증된 것도 시스템으로 실행된 것도 없습니다.",
    disclosureDetail:
      "이 페이지의 과제 정의는 실제 정의입니다. 사이트의 다른 곳에 있는 실행 수치는 예시용 데모 데이터이며 공식 순위가 아닙니다.",
    publicationStatus:
      "여기의 과제 정의는 공개 과제 세트의 잠정 후보입니다. 아직 어느 것도 검증(validated)되지 않았고 어떤 시스템으로도 실행되지 않았으며, 검증된 결과가 공개된 정의도 없습니다.",
    lifecycleEyebrow: "카탈로그 상태",
    lifecycleTitle: "확정된 과제 세트가 아니라 잠정 후보입니다",
    lifecycleIntro:
    ...
    lifecycleStatusDetail:
      "모든 과제는 후보(candidate) 상태입니다. 후보는 작성된 정의이고, 검증(validated) 과제는 아래 승격 요건을 통과한 과제입니다. 아직 승격된 과제는 없습니다.",
    lifecycleSetTerm: "공개 세트와 비공개 세트",
    lifecycleSetDetail:
      "이 페이지의 모든 과제는 공개 세트에 속합니다. 비공개(holdout) 세트는 구조적으로 분리해 보관하며 공개·내보내기·표시하지 않습니다. 여기 실린 과제는 비공개 세트로 재사용되지 않습니다.",
    ...
    promotionTitle: "검증된 과제로 인정되기 위해 갖춰야 하는 것",
    promotionIntro:
    ...
    scoringAggregationDetail:
      "유효한 개별 과제 점수들의 산술평균입니다. 시도되지 않았거나 유효하지 않거나 사전 등록된 기준값이 없는 과제는 사유와 함께 제외하며, 0으로 평균에 넣지 않습니다.",
  ...
  methodology: {
    ...
    scoringEyebrow: "채점",
    scoringTitle: "과제마다 최종 점수 하나, 세 축은 그대로 공개",
    scoringIntro:
      "점수는 요청 시 원본 기록에서 도출하므로, 자신이 요약한다고 주장하는 근거와 어긋날 수 없습니다. 아직 점수가 매겨진 것은 없습니다. 현재 과제 정의는 사전 등록 기준값이 없는 잠정 후보입니다.",
    scoringFormulaLabel: "과제별 최종 점수",
    scoringFormula: "100 × 정확도 × 속도 × 비용",
    scoringFormulaNote:
    ...
    scoringComponents: [
      ...
      },
      {
        term: "비용 성분",
        detail:
          "min(1, 비용 기준값 ÷ 관측 평가 실행 비용(USD))이며, 관측 비용이 영이면 1입니다. 어떤 경우에도 1을 넘지 않습니다.",
      },
    ],
    scoringAggregationTerm: "계열 점수와 국가 점수",
    scoringAggregationDetail:
      "해당 계열 또는 시장에서 유효한 개별 과제 점수들의 산술평균입니다. 기하평균을 쓰지 않고, 집계 단계에서 가중치도 적용하지 않습니다.",
