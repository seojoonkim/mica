...
export const ko = {
  ...
  site: {
    ...
    definition:
      "MICA는 버전을 고정한 소비자 에이전트 시스템을 아시아와 중동 6개 시장의 소비자 과제를 기준으로 평가합니다. 실행은 격리된 모의 환경이 아니라 현지 채널과 본인확인, 동의 절차, 결제 수단, 앱 전용 흐름을 지나갑니다. 기준을 통과한 과제는 정규화 정확도·정규화 속도·정규화 비용을 곱해 점수 하나를 받고, 세 축의 원값은 그 옆에 나란히 공개합니다.",
    edition: "프리뷰 0.1",
    demoNotice:
  ...
  home: {
    ...
    statSystems: "검증된 시스템",
    statRuns: "측정한 실행",
    countedNote: "빌드 시점에 원본 데이터에서 직접 집계한 값입니다.",
    stackEyebrow: "측정 단위는 개별 모델이 아니라 에이전트 시스템입니다",
    frameworkEyebrow: "MICA 에이전트 역량 프레임워크",
    frameworkTitle: "에이전트 시스템 전체를 평가하고, 결과를 측정하며, 원인을 진단합니다",
    frameworkIntro:
      "MICA는 API 연동 테스트가 아닙니다. 입력 이해부터 오케스트레이션, 모델 라우팅, 기억, 도구 사용, 현지화, 안전, 복구까지 조립된 소비자 에이전트 시스템의 기본 성능 전반을 평가합니다.",
    frameworkStages: [
      { label: "Test · 실제 조건을 평가한다", title: "정형화된 프롬프트를 넘어 실제 조건을 평가", detail: "현지 언어, 본인확인, 결제, 권한, 인계, 복구 조건과 실제 사용자 맥락 및 변동 제약을 포함해 시스템을 평가합니다." },
      { label: "Measure · 완료를 측정한다", title: "호출 성공이 아니라 확인된 완료", detail: "정확도·속도·비용 원값을 공개하고, 실행별 점수를 100 × 정규화 정확도 × 정규화 속도 × 정규화 비용으로 계산합니다." },
      { label: "Explain · 원인을 진단한다", title: "성공·실패 원인 진단", detail: "결과를 7개 시스템 역량 축으로 추적하되 진단값을 점수에 섞지 않습니다." },
    ],
    frameworkFieldTitle: "10개 소비자 과제 영역이 에이전트 시스템의 평가 범위를 구성합니다",
    frameworkFormulaLabel: "MICA 점수 = 100 × 정규화 정확도 × 정규화 속도 × 정규화 비용",
    frameworkFieldDetail:
      "100개 표준 과제가 6개 현지 시장에서 MICA의 평가 환경을 구성합니다. 10개 과제 영역은 예약·주문·변경·행정 등 복수 서비스를 포함하는 소비자 과제를 정의하며, 각 과제에는 명시된 완료 조건과 사용자 승인 경계가 적용됩니다. 에이전트 시스템은 입력 해석, 계획 수립, 모델·도구 선택, 상태 유지, 현지화, 안전한 인계 및 복구를 포함하는 하나의 종단 간 실행 단위로 평가됩니다. 실행 기록에서 확인된 최종 상태를 기준으로 정확도를, 성공한 실행의 경과 시간을 기준으로 속도를, 성공당 평가 실행 비용을 기준으로 비용을 산출합니다. 호출 계보와 상태 전이는 7개 진단 축에 연결해 성공·실패 원인을 분석합니다. 따라서 MICA는 서로 분리된 10개의 API 연동 테스트가 아니라 에이전트 시스템 전체를 평가하는 벤치마크입니다.",
    bandEyebrow: `${COUNTRIES.length}개 시장 에디션`,
    disclosureDetail:
      "인터페이스는 구축되었으나 지수에 포함된 검증 결과는 아직 없습니다.",
    publicationStatus:
      "MICA가 공개한 시스템 점수나 순위는 아직 없습니다.",
    noResultsHeadline: "아직 공개된 검증 결과가 없습니다",
    noResultsDetail:
    ...
    positionItems: [
      {
        term: "평가 단위",
        detail:
          "MICA의 측정 단위는 소비자 과제에 포함된 모든 서비스를 포괄하는 종단 간 실행 결과입니다. 개별 모델의 응답, 도구의 반환값, 단일 화면의 처리 성공은 독립적인 평가 단위가 아닙니다. 예약, 주문, 취소 또는 변경이 명시된 완료 조건에 도달해야 성공으로 인정합니다. 동일한 과제가 로그인 단계에서 중단되면 부분 성공이 아니라 실패로 판정합니다.",
      },
      {
        term: "시장이 곧 과제",
        detail:
    ...
    axesIntro:
      "정확도·속도·비용은 고유 단위 그대로 각각 공개합니다. 과제별 MICA 점수는 100 × 정규화 정확도 × 정규화 속도 × 정규화 비용으로 산출하지만, 참조값·반복 실행 계약·불확실성을 파일럿에서 보정하기 전에는 대표 수치나 순위 기준으로 사용하지 않습니다. 아직 점수를 받은 과제는 없습니다.",
    readinessEyebrow: "구축된 것과 아직 구축되지 않은 것",
    ...
    readinessItems: [
      ...
      },
      {
        term: "검증된 결과",
        detail:
          "없습니다. 측정된 제출물도, 재실행된 스냅샷도 없고, 어느 시장이나 계열에도 순위가 없습니다.",
      },
    ],
  ...
  countries: {
    ...
    standfirst:
      "국가 에디션은 하나의 벤치마크를 다른 언어로 옮긴 것이 아닙니다. 각 에디션은 그 시장에서 실행을 실제로 좌우하는 채널, 결제 인프라, 신원 확인, 주소 형식, 서비스 관행에 맞춰 별도로 작성합니다. 따라서 동일한 소비자 과제도 6개 시장에서 서로 다른 실행 조건을 갖습니다.",
  ...
  country: {
    ...
    familyIntro:
      "같은 시스템이라도 한 계열은 능숙하게 수행하고 다른 계열은 완료하지 못할 수 있습니다. 계열별 셀을 별도로 표시하는 이유입니다.",
    ...
    hazardsIntro:
      "각 운영상 난점은 관련 진단 축에 연결됩니다. 이를 통해 실패 원인을 일반적인 난이도가 아니라 특정 시스템 역량으로 분석할 수 있습니다.",
    localisationEyebrow: "현지화",
    localisationTitle: "과제가 현지의 것이 될 때 달라지는 것",
    localisationIntro:
      "에이전트가 이곳에서 일상 과제를 완료하려면 먼저 반영해야 하는 구체적인 차이입니다.",
  ...
  rankings: {
    ...
    overallIntro:
      "6개 시장에서 모두 유효한 커버리지를 확보한 시스템만 전체 등록부에 들어갑니다. 시장과 항목 수치는 그 안의 개별 과제 점수를 산술평균한 값입니다. 시장과 항목을 가로지르는 단일 종합 점수는 아직 정의되어 있지 않으며, MICA는 이를 만들기 위한 가중치를 임의로 설정하지 않습니다. 정확도 원값은 시장별 매크로 평균으로, 속도 원값은 성공 실행 기준으로 각각 확인하며, 통화를 합칠 수 없으므로 성공당 비용의 전체 순위는 만들지 않습니다.",
    ...
    scoreItems: [
      {
        term: "과제별 최종 점수",
        detail:
          "검증되어 실행 가능한 과제 시도 하나가 100 × 정규화 정확도 × 정규화 속도 × 정규화 비용을 받습니다. 각 인자는 0과 1 사이로 정규화된 성분이므로 점수는 0에서 100 사이에 놓입니다.",
      },
      {
        term: "항목 점수",
        detail:
  ...
  agents: {
    metaTitle: "시스템",
    metaDescription:
      "MICA 시스템 등록부에는 아직 제출·검증·공개된 시스템 버전이 없습니다.",
    eyebrow: "시스템 목록",
    ...
    emptyTitle: "등록된 시스템 없음",
    emptyNotice:
      "제출·검증·공개된 시스템 스냅샷은 아직 없습니다. MICA는 검증되지 않은 예시 시스템이나 수치를 표시하지 않습니다. 아래 항목은 향후 시스템이 등록될 때 공개되는 정보를 정의합니다.",
    labelsEyebrow: "공개 항목 정의",
  ...
  agent: {
    ...
    gateIntro:
      "MICA는 결과를 누락하지 않고 공개 불가 사유를 명시합니다.",
  ...
  tasks: {
    ...
    crossingTitle: "하나의 소비자 과제가 복수 서비스를 포함합니다",
    crossingIntro:
    ...
    scoringFormula: "100 × 정규화 정확도 × 정규화 속도 × 정규화 비용",
    scoringFormulaNote:
    ...
    scoringComponents: [
      {
        term: "정확도 성분",
        detail:
          "확인된 성공이면 1, 그 밖의 모든 결과는 0입니다. 부분 점수가 없으므로 더 빠르거나 더 저렴한 실패도 영점입니다. 곱셈 구조상 어느 한 성분의 영점이 최종 점수를 영점으로 만들기 때문입니다.",
      },
      {
        term: "속도 성분",
        detail:
          "min(1, 속도 기준값 ÷ 관측 초)이며, 해당 과제 버전에 사전 등록된 기준값과 비교합니다. 기준값보다 빨라도 1에서 멈추므로 속도로 정확도 손실을 상쇄할 수 없습니다.",
      },
      {
        term: "비용 성분",
        detail:
    ...
    scoringAggregationDetail:
      "적격 실행에서 파생한 과제 점수의 산술평균입니다. 사전 등록한 표준 과제 집합이 완결되지 않으면 점수를 비공표합니다. 제외 항목은 분모에서 제외된 사유와 함께 기록합니다.",
  ...
  evidence: {
    metaTitle: "근거",
    metaDescription:
      "MICA의 근거 등록부에는 아직 평가 결과 묶음이 없습니다. 측정된 시스템도 없습니다.",
    eyebrow: "실행 셀 계보",
    title: "근거",
    standfirst:
    ...
    emptyNotice:
      "근거 등록부에는 아직 기록이 없습니다. 측정된 시스템이 없으므로 공개할 집계와 계보도 없습니다. 아래 항목은 향후 MICA가 공개하는 모든 수치에 적용될 근거 요건을 정의합니다.",
    modelEyebrow: "실행 셀이란",
    modelTitle: "근거 모델",
    modelIntro:
    ...
    unitTerm: "단위",
    unitDetail:
      "시스템 × 시장 × 과제 계열마다 하나의 집계 실행 셀. 셀 id는 system--market--family 형식이므로, 모든 수치는 해당 수치가 산출된 구간까지 추적할 수 있습니다.",
    holdsTerm: "담고 있는 것",
    holdsDetail:
  ...
  evidenceCell: {
    ...
    identityIntro:
      "셀 ID는 안정적입니다. 시스템, 시장, 과제 계열을 이중 하이픈으로 연결한 값이며, 개별 실행이 아니라 집계를 가리키는 식별자입니다.",
    cellId: "셀 id",
    dataStatus: "데이터 상태",
    ...
    gateIntro: "MICA는 결과를 누락하지 않고 공개 불가 사유를 명시합니다.",
    limitsEyebrow: "한계",
    limitsTitle: "이 페이지가 보여 주지 않는 것",
    limitsIntro: "기록에 포함되지 않는 정보와 해석상의 한계를 명시합니다.",
    limits: [
      "개별 시도 기록은 없습니다. 표준 픽스처는 집계만 저장하므로 열어 볼 실행이 없습니다.",
      "타임스탬프, 대화 기록, 스크린숏, 도구 로그, 공급자 신원, 외부 근거 링크는 없습니다. 데모 에디션에는 그중 어느 것도 존재하지 않으며, 존재하지 않는 자료를 가리키는 참조를 생성하면 허위 출처 주장이 됩니다.",
  ...
  methodology: {
    ...
    scoringAggregationDetail:
      "해당 계열 또는 시장의 적격 실행에서 파생한 과제 점수의 산술평균입니다. 사전 등록한 표준 과제 집합이 완결되지 않으면 비공표합니다. 제외 항목은 분모에서 제외된 사유와 함께 기록합니다.",
    ...
    diagnosticsIntro:
      "MICA가 결과 원인을 분석하는 일곱 개의 진단 축입니다. 현재 프리뷰에서는 근거 항목만 정의하며 점수, 등급 또는 합산값을 제공하지 않습니다. 진단값을 산출할 근거가 아직 없기 때문입니다.",
    evidenceEyebrow: "근거",
    ...
    publicationIntro:
      "각 셀은 아래 조건을 모두 충족해야 합니다. 조건을 하나라도 충족하지 못한 셀은 차단 사유와 함께 표시됩니다. 두 조건의 수치 기준이 아직 확정되지 않았으므로 현재 프리뷰의 셀은 공개 요건을 충족할 수 없습니다.",
  ...
  governance: {
    ...
    correctionsIntro:
      "MICA는 오류가 발생할 수 있음을 전제로 합니다. 오류를 공개적으로 인정하고 신속하게 바로잡는 것이 MICA의 의무입니다.",
    corrections: [
      ...
      "철회된 셀은 철회 상태로 계속 표시합니다. 완전히 삭제하면 지수가 자신의 오류를 기록에서 제거하게 됩니다.",
    ],
    claimsEyebrow: "주장",
  ...
  submit: {
    ...
    accessTerm: "접근 수단",
    accessDetail:
      "MICA 통제 계정에서 시스템을 직접 실행할 수 있는 수단입니다. 이 수단이 없으면 결과는 자체 보고 단계로 제한됩니다.",
    traceTerm: "근거 추적",
    traceDetail:
    ...
    terms: [
      ...
      "금전 지급은 노출 위치, 포함 여부 또는 발표 시점에 영향을 주지 않습니다. 유료 등급은 운영하지 않습니다.",
    ...
    contactBody:
      "지수가 프리뷰인 동안 MICA에는 제출 접수 창구가 없습니다. 첫 공식 에디션이 열리면 접수 방법을 여기에 안내하고 에디션과 함께 공지합니다. 제출을 검토하는 기관은 방법론을 확인하고 오류 또는 개선 의견을 제시할 수 있습니다.",
    readMethodology: "방법론 읽기 →",
  ...
  families: {
    ...
    "shopping-delivery": {
      label: "쇼핑·배송",
      summary:
        "제약 조건에 맞는 특정 상품을 찾아 실제 현지 주소로 배송을 준비하고, 결제에 필요한 사용자 승인 이전 단계에서 중단하는 과제.",
      whyItIsHard:
    ...
    "travel-accommodation": {
      label: "여행 계획·숙박",
      summary:
        "복합 제약과 현지 재고 규칙을 충족하는 다구간 이동 일정 및 숙박 계획을 수립하는 과제.",
      whyItIsHard:
        "재고 공개 시점은 현지 일정 규칙의 영향을 받고, 일부 저비용 항공사는 통합 검색에서 제외됩니다. 검색 결과에 표시되지만 실제 예약이 불가능한 일정도 주요 실패 원인입니다.",
    },
    "restaurants-local": {
      label: "외식·예약",
      summary:
    ...
    "money-banking-investing": {
      ...
      whyItIsHard:
        "수수료, 환전 스프레드, 예금 조건의 고지 방식이 시장마다 다르고 최신성이 중요합니다. 가장 빠지기 쉬운 실패는 근거 없는 적합성 판단을 제시하는 확신에 찬 추천입니다.",
    },
    "mobility-transit": {
      label: "이동·대중교통",
      summary:
    ...
    "healthcare-administration": {
      label: "의료 행정",
      summary:
        "진료와 관련된 예약, 의뢰, 기록 요청, 보험 서류 및 비용 추정을 처리하는 행정 과제입니다. 임상 판단은 평가 범위에서 제외합니다.",
      whyItIsHard:
    ...
    "government-civic": {
      ...
      whyItIsHard:
        "규정은 권위 있으나 색인이 부실하고, 서식에는 판 번호가 있으며, 근거가 부족한 자격 판단은 답이 없는 것보다 나쁩니다. 기한과 신원 요건에는 예외가 없습니다.",
    },
    "home-utilities": {
      label: "주거·공과금",
      summary:
    ...
    "telecom-subscriptions": {
      ...
      whyItIsHard:
        "약정 조건, 단말 할부 및 번호 이동 기간은 계약 세부 조항에 포함되며, 구독 결제는 앱스토어와 카드에 분산될 수 있습니다. 대상 회선을 잘못 식별한 해지는 복구가 불가능할 수 있습니다.",
    },
  },
  ...
  diagnosticAxes: {
    ...
    "model-routing": {
      label: "모델 라우팅",
      description:
        "각 하위 단계를 적절한 모델로 라우팅하고, 경로를 사용할 수 없을 때 통제된 방식으로 성능을 저하시키는 역량.",
    },
    memory: {
      label: "메모리",
      description:
    ...
    recovery: {
      label: "복구",
      description:
        "단계 실패를 탐지·진단하고 대체 경로를 적용하거나, 일관된 상태를 유지한 채 실행을 중단하는 역량.",
    },
  },
  ...
  evidenceLabels: {
    ...
    recommendation: {
      label: "권고",
      description: "개발자와 소비자를 위한 권고이며 측정 결과와 구분됩니다.",
    },
  },
