...
export const ko = {
  ...
  site: {
    ...
    tagline:
      "6개 현지 시장에서 소비자 에이전트의 끝까지 실행을 평가하는 세계 최초의 공개 벤치마크 이니셔티브",
    secondary:
      "6개 현지 시장의 일상 과제를 끝까지 수행했는지 결과로 확인합니다.",
    definition:
    ...
    edition: "프리뷰 에디션 0.1",
    demoNotice:
      "예시용 데모 데이터이며 공식 순위가 아닙니다. 이 사이트의 수치는 화면을 만들려고 지어낸 값이라 근거가 되지 않습니다.",
  },
  ...
  home: {
    ...
    disclosureDetail:
      "MICA는 아직 만드는 중입니다. 화면은 완성했지만 그 뒤의 지수는 비어 있습니다. 확인을 마쳐 실은 결과가 없으니, 이 사이트의 수치는 어떤 제품의 성능도 말해 주지 않습니다.",
    publicationStatus:
      "지수는 아직 열지 않았습니다. MICA가 확인한 결과가 없으므로 이 페이지는 어떤 제품에도 점수나 순위를 매기지 않습니다.",
    noResultsHeadline: "아직 실은 결과가 없습니다",
    noResultsDetail:
      "MICA는 어떤 시스템도, 점수도, 순위도 낸 적이 없습니다. 지금 있는 것은 재는 틀뿐입니다. 아직 후보 단계인 10개 계열의 과제 분류, 공개해 둔 측정 방법, 기준값을 정하지 않은 공개 요건, 그리고 앞으로의 모든 수치를 이름 붙은 실행 묶음까지 되짚게 하는 근거 구조입니다. 제출한 시스템을 MICA가 직접 다시 돌려 이 요건을 통과해야 결과가 여기 올라옵니다.",
    positionEyebrow: "MICA를 만드는 이유",
    positionTitle: "소비자의 볼일이 끝났는지 재는 벤치마크가 없습니다",
    positionIntro:
      "그동안 벤치마크는 기업의 지식 노동, 소프트웨어 개발, 벤치마크가 통제하는 웹, 모바일이나 거래를 떼어 놓은 모의 환경에 몰려 있었습니다. 모두 분명한 성과이고 MICA도 그 위에 섭니다. 다만 조립된 소비자 에이전트가 살아 있는 현지 시장에서 일상 볼일을 끝내는지는 어느 쪽도 답하지 않습니다. MICA는 그 빈자리를 재려고 만드는 중이며, 아직 아무것도 재지 않았습니다.",
    positionItems: [
      {
        term: "빠져 있던 단위",
        detail:
    ...
    axesTitle: "정확도·속도·비용을 곱해 과제 점수를 냅니다",
    axesIntro:
      "기준을 통과한 과제는 점수 하나를 받습니다. 100 × 정확도 × 속도 × 비용이고, 각 인자는 초나 달러가 아니라 0과 1 사이로 맞춘 값입니다. 세 축은 원래 단위 그대로 따로 싣습니다. 점수는 기록을 줄여 보여 줄 뿐 기록을 대신하지 않습니다. 아직 점수를 받은 과제는 없습니다.",
    readinessEyebrow: "만든 것과 아직인 것",
    readinessTitle: "지금 어디까지 왔는가",
    readinessIntro:
      "MICA는 공개된 채로 만들어집니다. 그래서 빈 표로 넌지시 보이는 대신 각 부분이 어디까지 왔는지 적습니다. 아래 어느 항목도 측정 결과가 아닙니다.",
    readinessItems: [
      {
        term: "과제 분류",
        detail:
